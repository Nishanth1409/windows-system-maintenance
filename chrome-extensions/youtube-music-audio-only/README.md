...
Installed from: `C:\SystemMaintenance\chrome-extensions\youtube-music-audio-only`  
...
3. **Load unpacked** → select `C:\SystemMaintenance\chrome-extensions\youtube-music-audio-only`
...
```text
chrome-extensions/youtube-music-audio-only/
├── manifest.json           # MV3 — v1.20.0
